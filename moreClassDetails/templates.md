# 模板
**生成某个具体类型的模板（具体化模板代码）被称之为模板的实例化。**  
...
### 隐式实例化和显式实例化
- 隐式实例化
当我们使用了模板的某个实例时，这个实例会被隐式实例化。 如果多个源文件中使用了同一个模板的同一种版本类型，那么每个源文件在编译时都会被写入该实例版本的代码，虽然最后会进行截取从而保留唯一一个实例化代码，但是会增加编译的时间和成本。   
- 显式实例化
形式：  
...
>- 第一个阶段是编译模板本身。这个阶段编译器负责检查语法错误。（头文件并不被编译，编译的是被插入到使用该模本版的cpp。因此这里的编译模板指的是编译被插入到cpp文件的代码部分）    
>- 第二个阶段是编译器遇到模板使用时。对于函数模板，编译器通常检查参数数目是否正确，参数类型是否匹配。对于类模板，编译器可以检查用户是否提供了正确数目的模板实参（或许可以理解为ide的类型检查？只不过这里是编译过程中的类型检查）  。  
...
向该函数类型的指针。  
其他类型，如算术类型，派生类到基类，用户定义的转换，都不能应用于函数模板。但是不涉及模板类型参数的类型会进行正常的类型转换。
### 尾置返回类型和类型转换模板库
...
- 引用折叠和右值引用函数
> 1. 当我们将一个左值传递给右值引用函数，且右值引用指向模板类型参数（如：T&&），编译器会推断为T为int &   
